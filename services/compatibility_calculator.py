...
class CompatibilityCalculator:
    ...
    def get_student_score_with_specific_student(self, user_id, specific_student_id):
        ...
        user_data = {desc.category_id: {'answer': desc.answer, 'importance': desc.importance_score}
                     for desc in user_descriptions}
        specific_student_data = {desc.category_id: {'answer': desc.answer, 'importance': desc.importance_score}
        ...
        total_importance = 0
        category_scores = []
        for category_id, other_info in specific_student_data.items():
            if category_id in user_data:
                # Compute the similarity using Sentence Transformers
                user_answer = user_data[category_id]['answer']
                other_answer = other_info['answer']
        ...
        return {
            ...
            'categories': [{'category': category_dict[cat['category_id']], 'score': cat['adjusted_similarity'],
                            'answer': cat['answer']} for cat in category_scores]
        }
